...
renderCompoundStatement (Fundef dec idnt _ ws1 name ws2 params ws3 t s) =
  renderDec dec <>
  renderIndents idnt <>
  ...
  where 
    renderDec Nothing = mempty
    renderDec (Just (Decorator idts ident nl)) = 
      renderIndents idts <> singleton (TkAt (showRenderOutput $ renderIdent ident) ()) <> singleton (renderNewline nl)
...
renderParam (PositionalParam _ name t) =
  renderIdent name <> maybe mempty (\(_,t) -> singleton (TkColon ()) <> renderType t) t
...
renderParam (KeywordParam _ name _ ws2 expr) =
  renderIdent name <> singleton (TkEq ()) <>
...
renderBinOp (IntDivide _ ws) = TkDoubleSlash () `cons` foldMap renderWhitespace ws
