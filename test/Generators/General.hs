...
genParam genExpr =
  sizedRecursive
    ...
    [ KeywordParam () <$> genIdent <*> pure Nothing <*> genWhitespaces <*>
      genExpr
    ]
...
genModuleName =
  Gen.recursive
    ...
    [ModuleNameMany () <$> genIdent <*> genWhitespaces <*> genModuleName]

genRelativeModuleName :: MonadGen m => m (RelativeModuleName '[] ())
genRelativeModuleName =
  Gen.choice
    [ Relative <$> Gen.nonEmpty (Range.constant 1 10) genDot
    , RelativeWithName <$> Gen.list (Range.constant 1 10) genDot <*>
      genModuleName
    ]
...
genImportTargets =
  Gen.choice
    ...
    , ImportSome () <$> genSizedCommaSep1 (genImportAs genIdent genIdent)
    , ImportSomeParens () <$> genWhitespaces <*>
...
genCompFor =
  sized2M
    (\a b ->
       (\ws1 ws2 -> CompFor () ws1 a ws2 b) <$> genWhitespaces <*>
       genWhitespaces)
    genExpr
    genExpr
...
genCompIf =
  CompIf () <$> genWhitespaces <*> Gen.scale (max 0 . subtract 1) genExpr
...
genPyChar =
  Gen.choice
    ...
    , Char_uni16 <$> genHexDigit <*> genHexDigit <*> genHexDigit <*> genHexDigit
    , Char_uni32 <$> genHexDigit <*> genHexDigit <*> genHexDigit <*> genHexDigit <*>
      genHexDigit <*>
      genHexDigit <*>
      genHexDigit <*>
      genHexDigit
...
genExpr' isExp =
  sizedRecursive
    ...
    [ List () <$> genWhitespaces <*> Gen.maybe (genSizedCommaSep1' genExpr) <*>
      genWhitespaces
    , Dict () <$> genWhitespaces <*>
      sizedMaybe (genSizedCommaSep1' $ genDictItem genExpr) <*>
      genWhitespaces
    , Set () <$> genWhitespaces <*> genSizedCommaSep1' genExpr <*>
      genWhitespaces
    , ListComp () <$> genWhitespaces <*> genComprehension <*> genWhitespaces
    , Generator () <$> genComprehension
    , Gen.subtermM
        genExpr
        (\a -> Deref () <$> pure a <*> genWhitespaces <*> genIdent)
    , sized2M
        (\a b ->
           (\ws1 ws2 -> Call () a ws1 b ws2) <$> genWhitespaces <*>
           genWhitespaces)
        genExpr
        (sizedMaybe . genSizedCommaSep1' $ genArg genExpr)
    , sized2M
        (\a b ->
           (\ws1 ws2 -> Subscript () a ws1 b ws2) <$> genWhitespaces <*>
           genWhitespaces)
        genExpr
...
genSmallStatement =
  sizedRecursive
    ...
    , sized2M
        ...
        genExpr
        genExpr
    , Global () <$> genWhitespaces1 <*> genSizedCommaSep1 genIdent
    , Del () <$> genWhitespaces1 <*> genSizedCommaSep1' genExpr
    , Nonlocal () <$> genWhitespaces1 <*> genSizedCommaSep1 genIdent
    , Return () <$> genWhitespaces <*> sizedMaybe genExpr
    , Import () <$> genWhitespaces1 <*>
      genSizedCommaSep1 (genImportAs genModuleName genIdent)
    , From () <$> genWhitespaces <*> genRelativeModuleName <*> genWhitespaces <*>
      genImportTargets
    , Raise () <$> genWhitespaces <*>
      sizedMaybe
        ((,) <$> genExpr <*> Gen.maybe ((,) <$> genWhitespaces <*> genExpr))
    , sized2M
...
genModule =
  ...
  sizedList
    (Gen.choice
       [ fmap Left $
         (,,) <$> genIndents <*> Gen.maybe genComment <*> Gen.maybe genNewline
       , Right <$> genStatement
       ])

genImportAs ::
     MonadGen m => m (e ()) -> m (Ident '[] ()) -> m (ImportAs e '[] ())
